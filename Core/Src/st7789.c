...
// ── Toujours déclarés, DMA ou non ──────────────────
#define HOR_LEN    10
uint16_t disp_buf[ST7789_WIDTH * HOR_LEN];  // buffer global BSS
// ───────────────────────────────────────────────────
...
void ST7789_Init(void)
{
    ...
    HAL_Delay(10);
    ST7789_RST_Set();
    HAL_Delay(120);          // ← délai 120ms obligatoire après reset hardware

    ST7789_WriteCommand(ST7789_SLPOUT);    // 0x11 — Sleep Out OBLIGATOIRE
    HAL_Delay(120);          // ← délai 120ms obligatoire après SLPOUT
    ...
    ST7789_WriteCommand(0xD0);   ST7789_WriteSmallData(0xA4);
    ...
    ST7789_WriteCommand(ST7789_INVON);     // 0x21 — Inversion ON
    ST7789_WriteCommand(ST7789_NORON);     // 0x13 — Normal display ON
    ST7789_WriteCommand(ST7789_DISPON);    // 0x29 — Display ON
    HAL_Delay(10);
...
void ST7789_Fill_Color(uint16_t color) {
    uint16_t i, j;
    // Big-endian pour SPI : swap les bytes
    uint16_t color_be = ((color & 0xFF) << 8) | (color >> 8);

    // Remplir le buffer avec la couleur
    for (j = 0; j < ST7789_WIDTH * HOR_LEN; j++)
        disp_buf[j] = color_be;
    ...
    for (i = 0; i < ST7789_HEIGHT / HOR_LEN; i++)
        HAL_SPI_Transmit(&hspi2, (uint8_t*)disp_buf, sizeof(disp_buf), 1000);
    ST7789_UnSelect();
    printf("DEBUG ST7789_Fill_Color : Done\r\n");
}
 ...
 * @return none
 */
void ST7789_Fill(uint16_t xSta, uint16_t ySta,
