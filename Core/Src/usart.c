...
static SemaphoreHandle_t dataMutex = NULL; /* Serial data available */
...
void MX_USART1_UART_Init(void)
{
  ...
  taskHandle = osThreadNew(serialTask, NULL, &taskAttributes);
...
HAL_StatusTypeDef HAL_UART_Setup_UART1(void)
{
  ...
  ret = HAL_UART_DMAStop(&huart1);
  if (ret == HAL_OK)
    ret = HAL_UARTEx_ReceiveToIdle_DMA(&huart1, &uart1_rxbuf[0], APP_RX_DATA_SIZE);
  return ret;
}
...
HAL_StatusTypeDef HAL_UART_Setup_UART2(void)
{
  ...
  if (ret == HAL_OK)
    ret = HAL_UARTEx_ReceiveToIdle_DMA(&huart2, &uart2_rxbuf[0], APP_RX_DATA_SIZE);
  return ret;
}
